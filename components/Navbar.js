...
const WHATSAPP_NUMBER = '+919529085991' // Replace with your number
...
export default function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      scrolled ? 'bg-brand-dark/90 backdrop-blur-md border-b border-white/5' : ''
    }`}>
      ...
      {/* Mobile menu */}
      {menuOpen && (
        <div className="md:hidden bg-brand-dark-2 border-t border-white/5 px-6 py-4 flex flex-col gap-4">
          {links.map(link => (
            <a
              key={link}
              href={`#${link.toLowerCase()}`}
